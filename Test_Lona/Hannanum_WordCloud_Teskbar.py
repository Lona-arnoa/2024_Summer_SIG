...
# 검색어 입력 받기
user_request = input("사용자 질의를 입력하세요: ")
...
# 키워드 추출 및 카테고리 가져오기
output_filename = r'C:\Users\Lona\OneDrive\Desktop\SIG\workspace\2024_Summer_SIG-main\Test_Lona\HannanumList.txt'
        ...
        with urllib.request.urlopen(request) as response:
            if response.getcode() == 200:
                ...
                if items:
                    ...
                    category = category.replace('기타', '').replace('여성', '').replace('남성', '').replace('데코용품', '').replace('스티커', '').replace('케이크토퍼', '').strip()
                    return category
                return 'NO items'
...
def main():
    categories = []
    for content in tqdm(contents, desc="Processing contents"):
