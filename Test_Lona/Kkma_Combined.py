...
def split_noun_sentences(text):
    ...
    for sentence in sentences:
        if len(sentence) == 0:
            continue
        sentence_pos = kkma.pos(sentence)
        nouns = [word for word, pos in sentence_pos if pos == 'UN' or pos =='NNP' or pos =='NN'
                 or pos == 'NNB' or pos =='NNG' or pos == 'NNM']
        if len(nouns) == 1:
            continue
...
# 키워드 추출 및 카테고리 가져오기
output_filename = r'C:\Users\Lona\OneDrive\Desktop\SIG\workspace\2024_Summer_SIG-main\Test_Lona\KkmaList.txt'
